...
import 'package:seg_iv_drip/helper/image_segmentation_helper.dart';
...
class _MyHomePageState extends State<MyHomePage> with WidgetsBindingObserver {
  ...
  Future<void> _imageAnalysis(CameraImage cameraImage) async {
    ...
    // 🔍 Estimate FPS (based on time between frames)
    final now = DateTime.now();
    if (_lastFrameTime != null) {
      final diff = now.difference(_lastFrameTime!);
      final fps = 1000 / diff.inMilliseconds;
      debugPrint("Estimated FPS: ${fps.toStringAsFixed(1)}");
    }
    _lastFrameTime = now;

    // ⏱️ Start stopwatch before inference
    final stopwatch = Stopwatch()..start();

    final masks =
        await _imageSegmentationHelper.inferenceCameraFrame(cameraImage);

    // ⏱️ Stop and log inference time
    stopwatch.stop();
